...
class ContractTester {
  ...
  async getBalance(address: string): Promise<number> {
    try {
      ...
      return parseFloat(data.result[0].final_balance) || 0;
    } catch {
      return 0;
    }
  }
...
async function testPingAndAumFee(tester: ContractTester, ownerAddress: string) {
  ...
  assertApproxEqual(vaultAfter!.unlockDate, expectedUnlockDate, 60000, 'Unlock date should be reset');
...
async function testClaimMultipleHeirs(
  ...
  // Note: heir1 pays gas, so may receive less after fees
...
async function main() {
  ...
  const ownerProvider = Web3Provider.buildnet(ownerAccount);
  const ownerAddress = ownerAccount.address.toString();
  ...
  if (CONFIG.HEIR1_PRIVATE_KEY) {
    ...
    const heir1Provider = Web3Provider.buildnet(heir1Account);
    heir1Address = heir1Account.address.toString();
  ...
  if (CONFIG.HEIR2_PRIVATE_KEY) {
    ...
    const heir2Provider = Web3Provider.buildnet(heir2Account);
    heir2Address = heir2Account.address.toString();
  ...
  if (CONFIG.HEIR3_PRIVATE_KEY) {
    ...
    const heir3Provider = Web3Provider.buildnet(heir3Account);
    heir3Address = heir3Account.address.toString();
